...
def calculate_ap(IoUs_Metrics, gt_number, thre_list, device, confidence=None, function_select='integral'):
    ...
    def integral_method(prec, rec):
        ...
        """
        mrec = torch.cat((torch.Tensor([0.]).to(device), rec, torch.Tensor([1.]).to(device)))
        mprec = torch.cat((torch.Tensor([0.]).to(device), prec, torch.Tensor([0.]).to(device)))
    ...
    if confidence is not None:
        column_max_index = torch.argsort(confidence, descending=True)  # sorted indices of existing labels
    ...
    for thre in thre_list:
        ...
        tp_list = tp_list.to(device)
        precisions = torch.cumsum(tp_list, dim=0) / (torch.arange(len(tp_list)).to(device) + 1)
