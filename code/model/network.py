...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        # print(points_flat.max(axis=0))
        # print(points_flat.min(axis=0))
...
class SemImplicitNetwork(nn.Module):
    ...
    def gradient(self, x):
        ...
        # change it to support each sdf to satisfied eki loss
        return g
    ...
    def get_outputs(self, x):
        ...
        ''' Clamping the SDF with the scene bounding sphere, so that all rays are eventually occluded '''
        # apply bound at first
        if self.sdf_bounding_sphere > 0.0:
            sphere_sdf = self.sphere_scale * (self.sdf_bounding_sphere - x.norm(2,1, keepdim=True))
            sdf_raw = torch.minimum(sdf_raw, sphere_sdf.expand(sdf_raw.shape))
        # semantic = torch.softmax(-5*sdf_raw, dim = -1)
        # semantic = torch.softmax(-sdf_raw, dim=-1)
        # semantic = torch.sigmoid(-100*sdf_raw)
        # semantic = 4*torch.sigmoid(-50*sdf_raw)*(1-torch.sigmoid(-50*sdf_raw))
    ...
    def get_specific_outputs(self, x, idx):
        ...
        # sdf = -self.pool(-sdf_raw) # get the minium value of sdf
        # ''' Clamping the SDF with the scene bounding sphere, so that all rays are eventually occluded '''
        # if self.sdf_bounding_sphere > 0.0:
        #     sphere_sdf = self.sphere_scale * (self.sdf_bounding_sphere - x.norm(2,1, keepdim=True))
        #     sdf = torch.minimum(sdf, sphere_sdf)
...
class SemVolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.bg_color = torch.tensor(conf.get_list("bg_color", default=[1.0, 1.0, 1.0])).float().cuda()
        # self.bg_color = torch.tensor(conf.get_list("bg_color", default=[0, 0, 0])).float().cuda()
    ...
    def forward(self, input):
        ...
        # assert False
        # get semantic predict by normalized sdf
        ...
        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)
        # semantic_values = torch.log(torch.sum(weights.unsqueeze(-1)*semantic, 1).clamp(min=1e-6, max=1))
...
class SemImplicitNetwork_V2(nn.Module):
    ...
    def get_sdf_vals(self, x):
        ...
        if self.sdf_bounding_sphere > 0.0:
            sphere_sdf = self.sphere_scale * (self.sdf_bounding_sphere - x.norm(2,1, keepdim=True))
            sdf = torch.minimum(sdf, sphere_sdf)
        # sdf = -self.pool(-sdf) # get the minium value of sdf
        return sdf
...
class SemVolSDFNetwork_V2(nn.Module):
    ...
    def volume_rendering_with_threshold(self, z_vals, sdf, semantic, threshold):
        ...
        density = density_flat.reshape(-1, z_vals.shape[1])  # (batch_size * num_pixels) x N_samples
        # update density accroding to semantic and threshould
        # print(density.shape)
    ...
    def render_via_semantic(self, input, idx):
        ...
        semantic = semantic.reshape(-1, N_samples, self.num_semantic)
        # weights = self.volume_rendering(z_vals, sdf_raw[:, idx: idx+1])
        # weights = self.volume_rendering_with_threshold(z_vals, sdf, torch.softmax(semantic, dim=-1)[:, :, idx], 0.999999)
